...
@auth_bp.route("/login")
def login():
    return "<code><h1>login as user</h1> <small>not create yet</small></code>"
...
def institute_register():
    ...
    if form.validate_on_submit():
        # Check for existing institute with the same email
...
def institute_login():
    if current_user.is_authenticated:
        return redirect(url_for("institute_dashboard_bp.institute_dashboard"))
...
@auth_bp.route("/institute_logout")
def institute_logout():
    logout_user()
    return redirect(url_for("auth.institute_login"))  # Ensure this matches the endpoint
